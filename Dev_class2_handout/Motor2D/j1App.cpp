...
bool j1App::Awake()
{
	...
	pugi::xml_parse_result status=config_file.load_file("../Game/config.xml");

	bool ret = true;

	if (status)LOG("configuration xml file loaded correctly");
	else {
		LOG("config xmls file NOT loaded. parse error: ");
		LOG(status.description());
		ret = false;
	}
	node_config = config_file.first_child();
